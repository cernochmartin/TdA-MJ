...
<template>
    <section v-for="lecturer in body" :key="lecturer.lecturer_uuid" class="container pt-32">
        <h1 v-if="!editMode">{{ lecturer.title_before }} {{ lecturer.first_name }} {{ lecturer.middle_name }} {{ lecturer.last_name }} {{ lecturer.title_after }}</h1>
        <h1 v-else>
            ...
            <div class="w-2/3 flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input v-else type="text" :placeholder="lecturer?.price_per_hour + ` Kč`" v-model="teacherData.price_per_hour" />
                    </div>
                    <div class="flex flex-col gap-1">
        ...
        <div class="flex justify-end">
            <div class="flex gap-6">
                <button v-show="editMode" @click="editMode = !editMode" class="animation-up bg-error/70 w-[160px] py-2 rounded-md">Ukončit úpravu</button>
                <button v-if="editMode" @click="popup = true" class="animation-up bg-success w-[160px] py-2 rounded-md">Uložit úpravu</button>
                <button v-else @click="editMode = !editMode" class="animation-up bg-sunglow w-[100px] py-2 rounded-md">Upravit</button>
                <button @click="deleteLecturer()" class="animation-up bg-error w-[100px] py-2 rounded-md text-white">Vymazat</button>
            </div>  
        </div>
    </section>
